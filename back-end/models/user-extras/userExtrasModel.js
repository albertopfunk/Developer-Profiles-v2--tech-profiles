...
async function insert(userExtra, body) {
  ...
  if (dbEnv === "production") {
    const [id] = await db(`${userExtra}`).returning("id").insert(body);
...
function getSingle(userExtra, userExtraId) {
  return db(`${userExtra}`).where({ id: userExtraId }).first();
}

async function update(userExtra, userExtraId, body) {
  await db(`${userExtra}`).where({ id: userExtraId }).update(body);
  return getSingle(userExtra, userExtraId);
}

function remove(userExtra, userExtraId) {
  return db(`${userExtra}`).where({ id: userExtraId }).delete();
}
